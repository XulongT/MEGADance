...
class Metric():
    def __init__(self, root_dir='./data'):
        ...
        self.keypoints_static_pred = []
        self.keypoints_static_gt = []
    ...
    def update(self, result):
        self.smpl_poses_pred.append(result['smpl_poses_6d_pred'].detach().cpu())
        self.smpl_poses_gt.append(result['smpl_poses_6d_gt'].detach().cpu())
        ...
        self.keypoints_static_pred.append(result['keypoints_static_pred'])
        self.keypoints_static_gt.append(result['keypoints_static_gt'])
        ...
        # self.music_feature.append(result['music_feature'].detach().cpu())
    ...
    def result(self):
        ...
        keypoints_pred, keypoints_gt = torch.cat(self.keypoints_pred, dim=0), torch.cat(self.keypoints_gt, dim=0)
        keypoints_static_pred, keypoints_static_gt = torch.cat(self.keypoints_static_pred, dim=0), torch.cat(self.keypoints_static_gt, dim=0)
        ...
        # music_feature, music_beat = torch.cat(self.music_feature, dim=0), torch.cat(self.music_beat, dim=0)
        # print("MSE Loss of Vel: ", self.MSE_Loss(smpl_root_vel_gt, smpl_root_vel_pred))
        # print("MAE Loss of Vel: ", self.MAE_Loss(smpl_root_vel_gt, smpl_root_vel_pred))
        smpl_pred, smpl_gt = torch.cat([smpl_root_vel_pred, smpl_poses_pred], dim=2), torch.cat([smpl_root_vel_gt, smpl_poses_gt], dim=2)
        print("MSE Loss of SMPL: ", self.MSE_Loss(smpl_gt, smpl_pred))
        print("MAE Loss of SMPL: ", self.MAE_Loss(smpl_gt, smpl_pred))
        # print("MSE Loss of Keypoint: ", self.MSE_Loss(keypoints_pred, keypoints_gt))
        # print("MAE Loss of Keypoint: ", self.MAE_Loss(keypoints_pred, keypoints_gt))
        print("MSE Loss of Keypoint: ", self.MSE_Loss(keypoints_static_pred, keypoints_static_gt))
        print("MAE Loss of Keypoint: ", self.MAE_Loss(keypoints_static_pred, keypoints_static_gt))
...
def multimodality(music_feat, dance_feat):
    ...
    n, c = music_feat.shape
    music_feat, dance_feat = np.array(music_feat), np.array(dance_feat)
    music_feat = music_feat[:, np.newaxis, :]
    dance_feat = dance_feat[np.newaxis, :, :]

    diff = music_feat - dance_feat
    sq_diff = np.sum(np.square(diff), axis=2)
    distances = np.sqrt(sq_diff)
    np.fill_diagonal(distances, 0)
    average_distances = np.sum(distances, axis=1) / (n - 1)
    average_distances = np.mean(average_distances)
    return average_distances


def multimodal_distance(music_feat, dance_feat):
    music_feat, dance_feat = np.array(music_feat), np.array(dance_feat)
    sq_diff = np.square(music_feat - dance_feat)
