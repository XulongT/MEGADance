...
class Trainer(nn.Module):
    ...
    def comparison(self, data, device):
        
        smpl_trans_pred = data['smpl_trans_pred'].to(device).float()
        smpl_poses_pred = data['smpl_poses_pred'].to(device).float()
        smpl_trans_gt = data['smpl_trans_gt'].to(device).float()
        smpl_poses_gt = data['smpl_poses_gt'].to(device).float()
        music_librosa = data['music_librosa'].to(device).float()
        music_beat = data['music_librosa'][:, :, 33].clone()
        label = data['label'].to(device).long()

        from utils.utils import keypoint_from_smpl, rotation_6d_to_angle_axis
        keypoints_pred = keypoint_from_smpl(smpl_trans_pred, rotation_6d_to_angle_axis(smpl_poses_pred), self.fsq.smpl)
        keypoints_gt = keypoint_from_smpl(smpl_trans_gt, rotation_6d_to_angle_axis(smpl_poses_gt), self.fsq.smpl)
        ...
        result = {
            'smpl_poses_pred': torch.zeros(1, 1, 1).to(device).float(), 'smpl_trans_pred': torch.zeros(1, 1, 1).to(device).float(), \
            'keypoints_pred': keypoints_pred, 'dance_pred_genre_feature': dance_pred_genre_feature, \
            'dance_pred_kinetic_feature': dance_pred_kinetic_feature, 'dance_pred_geometric_feature': dance_pred_geometric_feature, \
            'smpl_poses_gt': torch.zeros(1, 1, 1).to(device).float(), 'smpl_trans_gt': torch.zeros(1, 1, 1).to(device).float(), \
            'keypoints_gt': keypoints_gt, 'dance_gt_genre_feature': dance_gt_genre_feature, \
            'dance_gt_kinetic_feature': dance_gt_kinetic_feature, 'dance_gt_geometric_feature': dance_gt_geometric_feature, \
            'music_beat': music_beat, 'label': label
        }
        loss = {
            'total': 0
        }
