...
def createTrainDataset(root_dir='./data', batch_size=32, stride=512, sample_len=1024, start=0, end=1):
    ...
    for file_name in tqdm(file_names):
        ...
        total_len = min(smpl_pose.shape[0], smpl_tran.shape[0], music_librosa.shape[0], music_muq.shape[0])//8*8
    ...
    train_dataloader = DataLoader(TrainDataset(smpl_poses, smpl_trans, music_librosas, music_muqs, labels, file_names), batch_size=batch_size, shuffle=True)
...
class TrainDataset(Dataset):
    def __init__(self, smpl_poses, smpl_trans, music_librosas, music_muqs, labels, file_names):
        ...
        music_muqs_mean, music_muqs_std = torch.mean(music_muqs, dim=(0, 1)), torch.std(music_muqs, dim=(0, 1))
        music_librosas_mean, music_librosas_std = torch.mean(music_librosas, dim=(0, 1)), torch.std(music_librosas, dim=(0, 1))
    ...
    def __getitem__(self, idx):
        ...
        music_muq = self.music_muq[idx]
        label = self.labels[idx]

        return {'smpl_trans': smpl_trans, 'smpl_poses': smpl_poses, 'smpl_root_vel': smpl_root_vel, \
                'music_librosa': music_librosa, 'music_muq': music_muq, 'label': label}
